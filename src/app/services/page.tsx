...
import { Check, Laptop, Users, Workflow, Database, ShieldCheck, Globe, BarChart } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background overflow-x-hidden">
      ...
      <main className="flex-1">
        ...
        <div className="container mx-auto px-8 md:px-16 lg:px-32 py-10 md:py-16">
            ...
            {categories.map((cat, i) => (
              <Card key={i} className="rounded-funky shadow-xl border-none bg-slate-50 group hover:bg-white transition-all duration-500 relative overflow-hidden" data-aos="fade-up" data-aos-delay={i * 100}>
                <div className="absolute top-0 right-0 p-6 opacity-5 group-hover:opacity-10 transition-opacity">
                  <cat.icon className="w-20 h-20 md:w-24 md:h-24" />
                </div>
                <CardHeader className="p-5 md:p-6 pb-2">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <CardTitle className="text-base md:text-xl font-bold text-primary font-headline italic tracking-tight">{cat.title}</CardTitle>
                      <p className="text-[8px] md:text-[9px] text-foreground/50 font-bold uppercase tracking-wider mt-0.5">{cat.description}</p>
                    </div>
                  </div>
                </CardHeader>
                  ...
                  <ul className="space-y-2 md:space-y-3">
                    {cat.features.map((feat, fi) => (
                      <li key={fi} className="flex gap-3 items-start text-foreground/80 group-hover:text-foreground">
                        <div className="mt-1 h-3 w-3 md:h-3.5 md:w-3.5 rounded-full bg-accent/20 flex items-center justify-center shrink-0 group-hover:bg-accent transition-colors">
                          <Check className="h-1.5 w-1.5 md:h-2 md:w-2 text-primary group-hover:text-white" />
                        </div>
                        <span className="text-[10px] md:text-xs font-medium">{feat}</span>
                      </li>
                    ))}
                  </ul>
                </CardContent>
          ...
          <section className="mt-8 p-6 md:p-10 bg-primary rounded-funky shadow-2xl relative overflow-hidden text-center text-white" data-aos="zoom-in">
            ...
            <h2 className="text-lg md:text-2xl font-bold font-headline mb-2">Scalable Hosting for Academic Excellence</h2>
            <p className="text-white/80 max-w-xl mx-auto mb-6 font-medium text-[9px] md:text-xs leading-relaxed">
            ...
            <div className="flex flex-wrap justify-center gap-4 md:gap-8 opacity-60 mb-6">
              <div className="flex flex-col items-center gap-1">
                <Globe className="h-4 w-4 md:h-5 md:w-5" />
                <span className="text-[6px] md:text-[8px] font-bold uppercase tracking-widest">Global Indexing</span>
              </div>
              <div className="flex flex-col items-center gap-1">
                <ShieldCheck className="h-4 w-4 md:h-5 md:w-5" />
                <span className="text-[6px] md:text-[8px] font-bold uppercase tracking-widest">Secure Servers</span>
              </div>
              <div className="flex flex-col items-center gap-1">
                <BarChart className="h-4 w-4 md:h-5 md:w-5" />
                <span className="text-[6px] md:text-[8px] font-bold uppercase tracking-widest">Growth Analytics</span>
              </div>
            </div>
            <Button asChild className="bg-accent hover:bg-white hover:text-primary text-accent-foreground px-6 py-2 text-[10px] md:text-xs font-bold rounded-funky shadow-xl w-full sm:w-auto h-auto transition-all">
