...
export function WhyUs() {
  return (
    <section id="why-us" className="py-8 md:py-12 bg-white overflow-hidden">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-8" data-aos="fade-up">
          <h2 className="text-2xl md:text-3xl font-bold text-primary font-headline">
            Why Choose Technical Journals?
          </h2>
          <div className="mt-1 w-16 h-1 bg-accent mx-auto"></div>
          <p className="mt-3 text-xs md:text-base text-foreground/80 max-w-2xl mx-auto font-medium">
            Empowering universities with professional, secure, and institutionally-owned publishing solutions on the Technical Journals platform.
          </p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
          {features.map((feature, index) => (
            <Card
              key={index}
              className="text-center p-5 bg-card shadow-lg rounded-funky transition-all duration-300 hover:shadow-2xl hover:-translate-y-1 border-border/50"
              data-aos="fade-up"
            ...
            >
              ...
              <p className="text-foreground/70 text-[11px] leading-relaxed font-medium">{feature.description}</p>
            </Card>
          ))}
        </div>
      </div>
